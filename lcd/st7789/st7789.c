...
void Send_Data8s(const void *data, size_t numbytes) {
    const uint8_t *p = data;
    while (numbytes--) {
        while (LL_SPI_IsActiveFlag_TXE(SPI1) == 0)
            ;
 ...
 * @return none
 */
static void ST7789_WriteData2(uint8_t *buff, size_t buff_size)
{
	ST7789_Select();
	ST7789_DC_Set();
...
static void ST7789_WriteData(uint8_t* buff, size_t buff_size)
{
    ...
    //}
    #ifdef USE_SPI_DMA
    Send_DMA_Data8(buff, buff_size);
    #else
    Send_Data8s(buff, buff_size);
    #endif
	ST7789_UnSelect();
}
/**
...
void ST7789_DrawFilledRectangle2(uint16_t x, uint16_t y, uint16_t w, uint16_t h, uint16_t color)
{
    // clipping
