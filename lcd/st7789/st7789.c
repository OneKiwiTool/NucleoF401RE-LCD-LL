...
#define TFT_CS_H() LL_GPIO_SetOutputPin(CS_PORT, CS_PIN)
#define TFT_CS_L() LL_GPIO_ResetOutputPin(CS_PORT, CS_PIN)
#define TFT_DC_D() LL_GPIO_SetOutputPin(DC_PORT, DC_PIN)
#define TFT_DC_C() LL_GPIO_ResetOutputPin(DC_PORT, DC_PIN)
#define TFT_RES_H() LL_GPIO_SetOutputPin(RST_PORT, RST_PIN)
#define TFT_RES_L() LL_GPIO_ResetOutputPin(RST_PORT, RST_PIN)
...
volatile uint32_t flag_DMA_STREAM3_bsy = 0;
...
static void Send_DMA_Data16(uint16_t* buff, uint16_t dataSize)
{
    ...
    LL_DMA_ConfigAddresses(DMA2, LL_DMA_STREAM_3, (uint32_t)buff, LL_SPI_DMA_GetRegAddr(SPI1)
        , LL_DMA_GetDataTransferDirection(DMA2, LL_DMA_STREAM_3));
    LL_DMA_EnableStream(DMA2, LL_DMA_STREAM_3);
    LL_SPI_Enable(SPI1);
    while (!flag_DMA_STREAM3_bsy) {
    }
    flag_DMA_STREAM3_bsy = 0;
                    ...
                    sFONT* Font, uint16_t Color_Background, uint16_t Color_Foreground)
{
    ...
    if (Xpoint > ST7789_TFTWIDTH || Ypoint > ST7789_TFTHEIGHT) {
                         ...
                         sFONT* Font, uint16_t Color_Background, uint16_t Color_Foreground )
{
    ...
    if (Xstart > ST7789_TFTWIDTH || Ystart > ST7789_TFTHEIGHT) {
    ...
    while (* pString != '\0') {
        ...
        if ((Xpoint + Font->Width ) > ST7789_TFTWIDTH ) {
            Xpoint = Xstart;
            Ypoint += Font->Height;
        }

        // If the Y direction is full, reposition to(Xstart, Ystart)
        if ((Ypoint  + Font->Height ) > ST7789_TFTHEIGHT ) {
            Xpoint = Xstart;
            Ypoint = Ystart;
        }
